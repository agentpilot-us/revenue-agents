...
export function ProductRowActions({ productId, productName }: Props) {
  ...
  const handleDelete = async (e: React.MouseEvent) => {
    e.preventDefault();
    if (deleting) return;
    if (!confirm(`Remove "${productName}" from your company data?`)) return;
    setDeleting(true);
    try {
      const res = await fetch(`/api/content-library/products/${productId}`, { method: 'DELETE' });
      if (!res.ok) {
  ...
  return (
    <div className="flex items-center gap-2 shrink-0">
      <Link
        href={`/dashboard/content-library/products/${productId}/edit`}
